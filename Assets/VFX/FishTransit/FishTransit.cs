...
namespace VFX
{
    public class FishTransit : MonoBehaviour
    {
        ...
        public Task StartTransit(Vector3 target, Color colour)
        {
            ...
            _MidBezierPoint = (_StartPosition + _TargetPosition) / 2.0f + 
                              Utility.Vector.AnyCross(_TargetPosition - _StartPosition).normalized * 5.0f;
        ...
        private void ProgressTransit(float t)
        {
            _CurrentTime = t;
            var bezierPos =
                Utility.Bezier.Construct(new List<Vector3>() { _StartPosition, _MidBezierPoint, _TargetPosition },
                    _CurrentTime);
