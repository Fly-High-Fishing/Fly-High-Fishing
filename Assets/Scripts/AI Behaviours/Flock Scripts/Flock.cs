...
{
    /* Variables */
    private List<FlockAgent> agents = new List<FlockAgent>(32); //allocate slots of memory
    ...
    void Update()
    {
        ...
        {
            List<Transform> neighbours = FindNearbyObjects(agent);

            velocity = behaviour.UpdatePosition(agent, neighbours, this);
    ...
    void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireSphere(thisPosition, spawnRadius); 
    }
    ...
    private void SpawnFlock()
    {
        for (int i = 0; i < agentsToSpawn; i++)
        {
            ...
            Vector3 spawnPosition = thisPosition + randomVector; //Sum the position with the random position
    ...
    {
        Vector3 agentPosition = agent.transform.position; //Cache agent position

        Collider[] neighbourColliders = Physics.OverlapSphere(agentPosition, MathsOperations.Square(GetPerceptionRadius));
        List<Transform> context = new List<Transform>(neighbourColliders.Length); //Prevent resizing

        foreach (Collider nc in neighbourColliders) {
            if (nc != agent.AgentCollider) //Saftey check for the current agents collider
            {
                float distanceToNeighbour = Vector3.SqrMagnitude(nc.transform.position - agentPosition);
