...
public class SeperationBehaviour : FilteredFlockingBehaviour
{
    ...
    private Vector3 neighbourPosition = Vector3.zero;
    ...
    public override Vector3 UpdatePosition(FlockAgent agent, List<Transform> neighbours, Flock flock)
    {
        ...
        foreach (Transform neighbour in filteredNeighbours) //For each neighbour in the list of neighbours
        {
            ...
            float distanceToNeighbour = Vector3.SqrMagnitude(neighbourPosition - agentPosition); //Calculate the distance between agent and the current neighbour
            if (distanceToNeighbour < flock.GetPerceptionRadius && distanceToNeighbour > 0.001F)              //If the current neighbour is within the current agents perception radius
            {
                ...
                direction = agentPosition - neighbourPosition;       //Offset of agents position and neighbours position
        ...
        if (Vector3.Magnitude(seperationSteer) > 0)
        {
            ...
            Vector3 agentVelocity = (agentPosition - previousPosition) / Time.deltaTime; //Calculate the approximate velocity of the agent
            previousPosition = agentPosition;                                           //Set the current position to be the previousPosition
